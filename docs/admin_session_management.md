...
**Why `activation-code.sh` fails**: Your `activation-code.sh` script generates a _random newly created UUID_ every time it runs, and sends it to the **Daily Login** endpoint. Because that random ID is not in the database, the backend correctly rejects it with a **404 Not Found (Device is not registered)**.
