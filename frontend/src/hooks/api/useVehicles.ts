...
import { VehicleSearchRequest, CreatePendingSubmissionRequest } from '../../openapi-rq/requests/types.gen';

export function useVehicles() {
    const searchMutation = useSearchVehicle();
    const submitMutation = useSubmitVehicle();

    const search = useCallback(async (request: VehicleSearchRequest) => {
    ...
    const submit = useCallback(async (request: CreatePendingSubmissionRequest) => {
