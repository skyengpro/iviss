...
export function useStabilityDetection({
    ...
    const [stableResult, setStableResult] = useState<{ plateNumber: string; confidence: number } | null>(null);
        ...
        (result: DetectionResult | null): { plateNumber: string; confidence: number } | null => {
            ...
            setHistory((prev) => {
                ...
                if (allMatch) {
                    ...
                    const avgConfidence = newHistory.reduce((sum, item) => sum + item.confidence, 0) / newHistory.length;
                    setStableResult({
