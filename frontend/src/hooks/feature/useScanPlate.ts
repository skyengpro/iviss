...
export function useScanPlate({
    ...
    const processFrame = useCallback(async (imageSrc: string) => {
        if (useDemoData) {
            ...
            if (demoStateRef.current.count >= 5) {
                demoStateRef.current.currentPlate = mockPlates[Math.floor(Math.random() * mockPlates.length)];
    ...
    const startLiveScan = useCallback((getScreenshot: () => string | null) => {
