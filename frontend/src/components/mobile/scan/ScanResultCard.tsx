            ...
            <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className={cn(
                        ...
                        {
                            'bg-status-critical/10 text-status-critical': detectedPlate.status === 'critical',
                            'bg-status-warning/10 text-status-warning': detectedPlate.status === 'warning' || detectedPlate.confidence < 80,
                            'bg-status-valid/10 text-status-valid': detectedPlate.status === 'valid' && detectedPlate.confidence >= 80,
                        }
                    )}>
                        {detectedPlate.status === 'critical' && <AlertCircle className="h-5 w-5" />}
                        {detectedPlate.status !== 'critical' && detectedPlate.confidence >= 80 && <CheckCircle className="h-5 w-5" />}
                        {detectedPlate.status !== 'critical' && detectedPlate.confidence < 80 && <AlertTriangle className="h-5 w-5" />}
                    </div>
                    <div>
