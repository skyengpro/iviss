...
use crate::{
    ...
    queries::vehicle_queries::{get_vehicle_with_owner_by_plate, get_vehicle_status_by_plate},
};
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid plate format",              body = AppError, 
        ...
        (status = 404, description = "Plate not found in registry",       body = AppError,     
...
pub async fn search_vehicle(
    ...
    let vehicle_row: crate::models::search_vehicle::VehicleRow = get_vehicle_with_owner_by_plate(&state.db, &plate)
        .await?
    ...
    let status_row: Option<crate::queries::vehicle_queries::VehicleStatusRow> = get_vehicle_status_by_plate(&state.db, &plate).await?;
