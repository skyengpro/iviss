...
impl ActivationService {
    ...
    pub async fn generate_and_store(&self, user_id: &Uuid) -> Result<String> {
        ...
        let value = serde_json::to_string(&entry).context("Failed to serialize activation entry")?;
    ...
    pub async fn validate(&self, user_id: &Uuid, submitted_code: &str) -> Result<()> {
        ...
        // Retrieve the entry
        let raw: Option<String> = conn.get(&key).await.context("Failed to get activation entry from Redis")?;
        ...
        if entry.attempts >= MAX_ATTEMPTS {
            // Delete the key — the agent will need to request a new code
            conn.del::<_, ()>(&key).await.ok();
            return Err(anyhow::anyhow!("Max attempts reached — activation code invalidated"));
        }

        // Increment attempts before verification (brute-force protection)
        entry.attempts += 1;
        ...
        // Success — delete the key (one-time use)
        conn.del::<_, ()>(&key).await.ok();
